...
char SSID_buffer[40];
char PASS_buffer[40];
char SSID_buffer_eep[40];
char PASS_buffer_eep[40];
...
void int_setup() {  ////akkk
  ...
  writeStringToEEPROM(200, "001-Octane Ron(92)");
...
void wifi_init() {
  ...
  WiFi.begin(SSID_buffer_eep, PASS_buffer_eep);
...
void eeprom_init() {  // to read wifi data and pump id from eeprom
  ...
  readStringFromEEPROM(0, SSID_buffer_eep, sizeof(SSID_buffer));
  readStringFromEEPROM(40, PASS_buffer_eep, sizeof(SSID_buffer));
  ...
  dispenser_5_price = eeprom_read_data(127);
  ...
  readStringFromEEPROM(320, Fuel_Type_5, sizeof(Fuel_Type_5));
  readStringFromEEPROM(350, Fuel_Type_6, sizeof(Fuel_Type_6));
  readStringFromEEPROM(380, Fuel_Type_7, sizeof(Fuel_Type_7));
  readStringFromEEPROM(410, Fuel_Type_8, sizeof(Fuel_Type_8));
  ...
  Serial.println(SSID_buffer_eep);
  Serial.print("EEPROM read pass is => ");
  Serial.println(PASS_buffer_eep);
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(topic) == String(FuelType_topic)) {
    ...
    writeStringToEEPROM(320, fuelType_5);
    writeStringToEEPROM(350, fuelType_6);
    writeStringToEEPROM(380, fuelType_7);
    writeStringToEEPROM(410, fuelType_8);

    readStringFromEEPROM(200, PASS_buffer, sizeof(SSID_buffer));
...
void price_show_hmi() {  // to show price input from server
  ...
  if (pumpid1 == pumpmqttnum) {  // Pump Id 1
    ...
    eeprom_writeInt(127, dispenser_5_price);
    int Pump_Id = 5;
